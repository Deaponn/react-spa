    ...
    return (
        <TableContainer component={Paper}>
            <Table sx={{ minWidth: 350 }} aria-label="simple table">
                ...
                <TableBody>
                    {
                    itemList.length !== 0 && filterValue !== ""
                        ? itemList.filter((item) => item.id === parseInt(filterValue)).map((item) => <Item key={item.id} data={item} />)
                        : itemList.map((item) => <Item key={item.id} data={item} />)}
                </TableBody>
            </Table>
